...
export default function DashboardAdmin() {
  ...
  const rentBuyData = [
    { name: 'For Rent', value: platformStats.rent, color: '#2563eb' },
    { name: 'For Sale', value: platformStats.buy, color: '#f97316' }
  ];
      ...
      <div className="bg-slate-900 text-white py-10 px-4 sm:px-6 lg:px-8 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1 bg-indigo-500/20 text-indigo-400 text-[10px] font-bold px-2 py-0.5 rounded uppercase border border-indigo-500/20">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className={`px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 ${
                activeTab === 'verification' ? 'bg-primary text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
            ...
            <button 
              ...
              className={`px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 ${
                activeTab === 'analytics' ? 'bg-primary text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        {activeTab === 'verification' && (
          <div className="space-y-6">
            <h3 className="font-display font-bold text-slate-800 text-lg">Pending Listing Audits</h3>
            ...
            {pendingVerifications.length === 0 ? (
              <div className="bg-white rounded-3xl border border-slate-200 p-12 text-center max-w-lg mx-auto space-y-4">
                ...
                <h3 className="font-display font-bold text-slate-800 text-base">All listings verified</h3>
                <p className="text-slate-500 text-xs font-semibold">There are no pending listings or owner verifications in the pipeline.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {pendingVerifications.map(p => (
                  <div 
                    key={p.id}
                    className="bg-white rounded-3xl border border-slate-200 p-6 flex flex-col md:flex-row justify-between gap-6 shadow-sm items-start md:items-center"
                  >
                    {/* Listing Summary */}
                    <div className="flex gap-4 items-start">
                      <img src={p.images[0]} alt={p.title} className="h-16 w-24 object-cover rounded-xl border shrink-0" />
                      <div className="space-y-1">
                        <h4 className="font-bold text-xs sm:text-sm text-slate-800 line-clamp-1">{p.title}</h4>
                        ...
                        <div className="flex gap-1.5 pt-1">
                          {!p.verifiedProperty && (
                            <span className="text-[9px] font-bold bg-orange-50 text-orange-600 px-2 py-0.5 rounded border border-orange-100 uppercase">
                              Docs Pending Audit
                            </span>
                          )}
                          {!p.verifiedOwner && (
                            <span className="text-[9px] font-bold bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded border border-indigo-100 uppercase">
                    ...
                    <div className="flex flex-wrap gap-2 w-full md:w-auto">
                      {!p.verifiedProperty && (
                        <button
                          ...
                          className="flex-1 md:flex-none px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-1"
                        >
                          <Check className="h-4 w-4" /> Verify Property
                        </button>
                      )}
                      {!p.verifiedOwner && (
                        <button
                          onClick={() => verifyOwnerListing(p.id)}
                          className="flex-1 md:flex-none px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-1"
                        >
                      ...
                      <button
                        ...
                        className="px-3 py-2 border border-slate-200 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition"
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm text-left">
                ...
                <h3 className="font-display font-black text-2xl text-slate-800">{platformStats.total}</h3>
              </div>
              <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm text-left">
                <p className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Rent vs Buy Ratio</p>
                <h3 className="font-display font-black text-xl text-slate-800">{platformStats.rent} : {platformStats.buy}</h3>
              </div>
              <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm text-left">
                <p className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Verified properties</p>
                <h3 className="font-display font-black text-2xl text-emerald-600">
              ...
              <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm text-left">
                ...
                <h3 className="font-display font-black text-2xl text-slate-800">1,240</h3>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm lg:col-span-2 text-left">
                <h4 className="font-display font-bold text-sm text-slate-800 mb-4">User Registration Growth (YTD)</h4>
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={userGrowthData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="Users" stroke="#2563eb" strokeWidth={2.5} name="Seekers" />
                      <Line type="monotone" dataKey="Owners" stroke="#10b981" strokeWidth={2} name="Owners/Brokers" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm text-left">
                <h4 className="font-display font-bold text-sm text-slate-800 mb-4">Rent vs Sale Distribution</h4>
                <div className="h-48 flex items-center justify-center">
                  ...
                  <div className="space-y-1 shrink-0 w-24 pl-2">
                    {rentBuyData.map((item, idx) => (
                      <div key={idx} className="text-[10px] font-bold text-slate-600 flex items-center gap-1.5">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm text-left">
              <h4 className="font-display font-bold text-sm text-slate-800 mb-4">Listings Distribution by City</h4>
              <div className="h-64">
